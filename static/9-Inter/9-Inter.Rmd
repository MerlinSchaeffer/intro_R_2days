---
title: "Intro to R"
subtitle: "9 Interaction Effects and Polynomials"
...
ESS <- ESS %>% transmute( # Create new variables and keep only those
  ...
  dplyr::filter(
    facntr == "Yes" & mocntr == "Yes" &
      # filter cases from three countries,
...
1. How do we interpret the estimate `eduyrs:cntryBulgaria` correctly?

2. What does $\beta_{\text{eduyrs}}$ refer to?
...
ggplot(data = synth_data, 
  ...
  labs(x = "Age",
  y = "Xenophobia",
  caption = "Note: OLS estimates. Covariates set to: Women in France at mean levels of (parental) education.") + 
  theme_minimal()
```
]
.panel[.panel-name[Code]
...
ggplot(data = synth_data, 
  ...
  y = "Xenophobia",
  caption = "Note: OLS estimates. Covariates set to: Women in France at mean levels of age and education.") + 
  theme_minimal()
```
]
.panel[.panel-name[Code]
...
ggplot(data = synth_data, aes(y = fit, x = par_edu)) +
  ...
  labs(x = "Parental education", y = "Xenophobia", 
       caption = "Note: OLS estimates. Covariates set to: Women at mean levels of age and education.") + 
  theme_minimal()
```
]]]]
---
...
ols7 <- lm_robust(formula = imwbcnt ~ log(par_edu) + 
                    gndr + poly(agea, 2) +
                    eduyrs*cntry, #<<
                  data = ESS, weights = pspwght)
...
ggplot(data = synth_data, 
  ...
  labs(x = "Years of education",
  y = "Xenophobia",
  caption = "Note: OLS estimates. Covariates set to: 
  Women at mean levels of age and parental education.") + 
  theme_minimal()
```
]
.panel[.panel-name[Code]
