...
subtitle: "Linear OLS Regression II"
...
ggplot(data = ESS, 
  ...
  labs(x = "Years of education", y = "imwbcntphobia", size = "Survey \n weight", color = "Country") +
  theme_minimal()
```
]]
...
1. How do we interpret the estimates correctly?

2. Why is $\beta_{\text{eduyrs}}$ smaller than in model 3?

3. Why do Bulgarians suddenly seem to have *lower* levels of imwbcntphobia, whereas before they had higher ones?
]]
...
ggplot(data = ESS, 
  ...
  geom_smooth(aes(weight = pspwght), 
              method = "lm", se = FALSE) +
  labs(x = "Years of education \n (z-standardized)", y = "imwbcntphobia", size = "Survey \n weight", color = "Country") +
  theme_minimal()
```
]
.panel[.panel-name[Plot]
...
ggplot(data = synth_data, aes(y = fit, x = par_edu)) +
  ...
  geom_line() +
  labs(x = "Parental education", y = "imwbcntphobia", 
       caption = "(Covariates set to: Women at mean levels of parental education)") + 
  theme_minimal()
```
]]]]
...
ggplot(data = synth_data, aes(y = fit, x = eduyrs)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr, 
              ...
              alpha = 0.5) +
  geom_line(aes(color = cntry)) + #<<
  labs(x = "Years of education", y = "imwbcntphobia", 
       caption = "(Covariates set to: Women at mean levels of parental education)") + 
  theme_minimal()
```
]]
