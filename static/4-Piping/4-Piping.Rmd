...
subtitle: "Piping & Grouped Operations"
...
Or even easier, think of it as: .alert["then"] or .alert["så"].
...
ESS <- ESS %>% # Import the ESS, then (pipe result into) #<<
  ...
  dplyr::filter(# Keep only respondents of native-born parents,
    facntr == "Yes" & mocntr == "Yes",
    # filter cases from four countries
    cntry == "Germany" | cntry == "Denmark" | 
      cntry == "France" | cntry == "Netherlands")
```
]
---
...
.push-left[
```{r out.width='80%', fig.width = 6, fig.height = 3.5}
ggplot(data = ESS, 
...
```{r out.width='80%', fig.width = 6, fig.height = 3.5}
ggplot(data = ESS, 
       mapping = aes(y = pspwght, x = eduyrs)) +
  geom_smooth() # Smoothed average weight by education.
```
]
...
voted_yes <- c(0, 1, 1, 1, 0)
weight <- c(2, 2, 1, 1, 1)

dat <- tibble(i, gender, vote, voted_yes, weight)
...
.content-box-blue[
$$\frac{3}{5} * 100 = 60$$
What is the patriarchy-weighted percent of "Yes" votes?
]
]
]
...
```{r}
# Three ways to the percent of yes votes:

((0*2 + 1*2 + 1 + 1 + 0) / 7) * 100 # 1

(sum(voted_yes * weight) / sum(weight)) * 100 # 2

Hmisc::wtd.mean(x = voted_yes, weights = weight) * 100 # 3
```
]
