...
2) Regress religiosity on age and make a regression table that displays 3 digits and standard errors in parentheses using `texreg::screenreg()`; $\beta$ = `r fitb(0.050, width = "5")` and the relation is statistically `r mcq(c("Insignificant", "Interesting", "Important", answer = "Significant"))`.
`r hide("R solution -> dont' peek to early ;) !")`
```{r ref.label = "ols1"}
```
`r unhide()`
...
4) Regress religiosity on gender and make a regression table that displays 3 digits and standard errors in parentheses using `texreg::screenreg()`; $\beta$ = `r fitb(0.789, width = "5")` and the relation is statistically `r mcq(c("Insignificant", "Interesting", "Important", answer = "Significant"))`.
`r hide("R solution -> dont' peek to early ;) !")`
```{r ref.label = "ols2"}
```
`r unhide()`
