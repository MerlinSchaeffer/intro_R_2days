...
subtitle: "Linear OLS Regression I"
...
ggplot(data = ESS, mapping = aes(y = imwbcnt, x = eduyrs)) +
  ...
  labs(y = "imwbcntphobia", x =  "Years of education", size = "Survey \n weight") +
  theme_minimal()
```
]
.panel[.panel-name[Code]
...
ggplot(data = ESS, mapping = aes(y = imwbcnt, x = eduyrs)) +
  ...
  geom_smooth(aes(weight = pspwght), method = "lm", 
              se = FALSE, color = "red") +
  labs(y = "imwbcntphobia", x =  "Years of education", 
       size = "Survey \n weight") +
  theme_minimal()
```
]]]
...
$$`r extract_eq(ols, use_coefs = TRUE)`,$$ is the line that $\min \sum_{i=1}^{n} e_{i}^{2}$.
...
dagify( # Define the DAG
  ...
  Edu ~ Par_edu + Conf,
  # Add labels
  labels = c("imwbcnt" = "imwbcntphobia", 
...
ggplot(data = ESS, 
  ...
  scale_x_continuous(breaks = c(0, 1), 
                     labels = c("0 = Denmark", 
                                "1 = Sweden")) +
  labs(x = "Country", y = "imwbcntphobia", 
       size = "Survey \n weight") +
  theme_minimal()
```
]
